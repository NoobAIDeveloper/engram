...
1. Read `<kb>/.engram/autosync.json`. If missing, tell the user no autosync is scheduled.
...
3. Also report the timestamp of the most recent successful sync by reading `<kb>/.engram/sync-meta.json` (X) and `<kb>/.engram/chatgpt-sync-meta.json` / `claude-ai-sync-meta.json` if they exist.
