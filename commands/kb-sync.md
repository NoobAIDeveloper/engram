...
The user wants to sync data into the current KB. The current working directory should be a KB (it should contain a `CLAUDE.md` and a `.engram/` subdirectory). If it doesn't, tell the user to `cd` into their KB first or run `/kb-init` to scaffold one.

Run the sync dispatcher:

```bash
~/.claude/skills/engram/.venv/bin/python ~/.claude/skills/engram/scripts/sync.py --kb $(pwd) $ARGUMENTS
```
...
- **github-stars** — Public GitHub stars for the handle in `.engram/sources.json`. Set `GITHUB_TOKEN` env for higher rate limit.
