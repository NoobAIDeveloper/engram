...
- If `.twitter-wiki/cluster-map.yaml` does not exist, do the bootstrap step (sample bookmarks, derive topics, write the map, confirm with the user) before running preprocess.
- Run `uv run ~/.claude/skills/twitter-wiki/scripts/preprocess.py --kb $(pwd)` once the map is in place.
